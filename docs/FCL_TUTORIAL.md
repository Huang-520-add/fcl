# FCL 超详细中文教学书（FCL Textbook — Chinese Edition）v3.0.2
...
**一句话概括 FCL**：把整个编程世界重新用生态学语言描述了一遍。它是深奥语言（Esolang），但不是玩具——它**试图用生态隐喻覆盖全部编程概念**（注意：变量集由源码静态声明、运行时不可动态分配，内存有界，**未严格证明图灵完备**）。
...
> **类比**：一副扑克牌理论上可以玩任何牌类游戏（斗地主、桥牌、21点……），但并不是每副牌都方便——有的规则很别扭，但功能是齐全的。FCL 也是这般——一套规则别扭、却具备关键积木的系统；不过它内存有界，图灵完备性并未被严格证明。
...
| **（有界的）变量集** | `INTRODUCE`（源码中声明**固定、有限**数量的物种）| 变量列表 |

**三件套都齐了，但因为存储有界（运行时无法动态分配新变量名），FCL 是否图灵完备并未被严格证明**。

---

## 15.3 FCL 的局限

FCL 具备了图灵完备系统的三件套，但内存有界使其图灵完备性未被证明；即便如此，它也不"好用"：
...
> 教学书 v3.0.2 ｜ 参考实现：[Huang-520-add/fcl](https://github.com/Huang-520-add/fcl)
