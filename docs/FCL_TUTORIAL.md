# FCL 超详细中文教学书（FCL Textbook — Chinese Edition）v3.1.0
...
**一句话概括 FCL**：把整个编程世界重新用生态学语言描述了一遍。它是深奥语言（Esolang），但不是玩具——它**试图用生态隐喻覆盖全部编程概念**；自 v3.1.0 起内置无界存储带（TAPE）与无界循环（WHILE），已**图灵完备**（与 Brainfuck / 图灵机等价）。
...
> **类比**：一副扑克牌理论上可以玩任何牌类游戏（斗地主、桥牌、21点……），但并不是每副牌都方便——有的规则很别扭，但功能是齐全的。FCL 也是这般——一套规则别扭、却具备关键积木的系统；自 v3.1.0 起它已图灵完备（无界存储带 TAPE + 无界循环 WHILE，与 Brainfuck / 图灵机等价）。

---

## 15.2 FCL 实现图灵完备的积木组合

| 能力 | FCL 实现 | 相当于 Brainfuck / 普通语言的 |
|---|---|---|
| **无界存储带** | `TAPE`（稀疏 map，容量无限） | 图灵机读写带 / 数组 |
| **读写头移动** | `FORWARD` / `BACKWARD` | `>` / `<` |
| **算术** | `BUMP <n>`（在带元上增减） | `+` / `-` |
| **细胞读写** | `LOAD` / `STORE`（带元 ↔ 物种能量） | `,` / `.` |
| **无界循环** | `WHILE TAPE UNTIL <expr>` | `[ ... ]` |

**上述积木齐备：** 无界存储带（`TAPE`）+ 读写头移动（`FORWARD`/`BACKWARD`）+ 算术（`BUMP`）+ 读写（`LOAD`/`STORE`）+ 无界循环（`WHILE`），与 Brainfuck / 图灵机等价，因此 **FCL 自 v3.1.0 起图灵完备**。

---

## 15.3 FCL 的局限

FCL 自 v3.1.0 起已图灵完备，但语法"别扭"，并不"好用"：
...
> 教学书 v3.1.0 ｜ 参考实现：[Huang-520-add/fcl](https://github.com/Huang-520-add/fcl)
