...
| FCL 特性 | 生态学依据 |
|---|---|
| 20% 能量传递效率 | **林德曼定律**（十分之一定律）：能量沿食物链传递效率约 10%~20%，其余经呼吸散失；能量单向流动、逐级递减 |
...
通用规则：
- **能量传递效率 20%**（俗称"能量税"，税率 80%）：非豁免场景下 `实际结果 = 理论结果 × 0.2`（热力学第二定律）
...
| 模式 | 声明 | ROT 行为 |
|---|---|---|
| **编码模式**（默认） | 无需声明 | 第一次 ROT 输出 `char(int(能量))`（ASCII 字符）；连续第二次输出 `U+XXXX`（Unicode 码位） |
| **数值模式** | `NUMERIC OUTPUT ;`（首行或任意位置） | 每次 ROT 直接输出数值本身（能量 55 → 输出 `55`） |

- `NUMERIC OUTPUT ;` 声明后对后续所有 ROT 生效（全局模式切换）
- GMO 模式下两种输出前均打印 🧬 标识（**仅 REAL MODE**；CODE 模式不显示）
- 教学提示：编码模式适合"字符艺术"，数值模式适合"看真实计算结果"（如三角数 55 在编码模式显示为字符 '7'，数值模式直接显示 55）
...
- **STORM 为显式开关**（v2.2 起）：默认 RAIN 块内语句数 >3 时**不会**乱序（保证确定性）。仅在程序首行声明 `STORM ENABLED ;` 后，RAIN 块语句数 >3 才降级为 STORM，块内语句随机乱序执行一次（🌩️）。**注意：STORM 引入非确定性，可能破坏依赖执行顺序的程序，请谨慎启用。**
...
定次循环。**副作用（v2.2 明确文档化）**：每轮迭代后该物种能量**开平方根**（模拟长途迁徙消耗）——能量 100 → 10 → 3.16 → 1.78 → 1.33。若需循环体内变量能量保持稳定，请勿将其作为 MIGRATION 的物种（可选用每轮重置的变量，如 INTRODUCE 覆盖为 0 的累加器）。
...
**条件循环（while）**：循环执行直到指定 APEX 变量为 FULL（1）。**副作用（v2.2 明确文档化）**：每轮该物种能量 ×0.9（冬眠代谢消耗 10%）——初始能量 100 经 10 轮后剩 34.8。超过 10000 轮强制唤醒（⏰）。

### 6.11 变异：MUTATION

```ebnf
MUTATION <物种名或成员名> { CASE "特征": <语句> ... } ;
```

以 1/3 概率对该**物种**发生变异：根名替换为同音异形词（Wolf→Wolv、Sheep→Sheepe、Tiger→Tygre、Bacillus→Bacilluz，全部 10 个在册物种见生态圈图鉴）。变异的效应：

- **物种级改名**：块执行期间，该物种全部在册成员临时改名（`Wolf_M1`→`Wolv_M1`、`Alpha_Wolf`→`Alpha_Wolv`），块内对该物种的引用同步改写；块结束后恢复原名，块外引用不受影响
- **分支选择**：变异触发时从所有 `CASE` 分支中**等概率随机表达一支**；未触发则整个块空转（`MUTATION ≈ switch + 随机 case`）
- **可检测**：`MATCH(name)` 按物种根名判定该物种是否发生过变异（根名/成员名/变异后名称均可），记录跨块持续存在

注意：变异名（如 `Wolv`）不在生态圈在册名录中，无法用于 `INTRODUCE` 引种。
...
- 能量税降为 0%（100% 传递效率）
- 代价：REAL MODE 下每次 ROT 输出前打印 🧬 转基因产品标识（CODE 模式不显示）

---

### 6.14 运行模式：REAL MODE / CODE MODE

```ebnf
REAL MODE ;    (* 真实模式：生态等待全部生效 *)
CODE MODE ;    (* 代码模式：跳过等待，高速执行（默认） *)
```
...
- 命令行 `fcl --real 程序.fc` 等价于程序内声明 REAL MODE
- 生态依据：真实生态中能量流动有耗时（进食、分解、消化），代码模式是"快进"视角
...
2. **尸体分解**：能量 =0 且连续 3 条指令未被引用 → DECOMPOSE，真实模式下随机阻塞 100~1000ms（代码模式即时）
...
所有错误均为结构化输出：`[错误码 @行号] 生态报错`（v2.2 起）。

| 错误码 | 传统报错 | FCL 报错 | 触发场景 |
|---|---|---|---|
| FCL-0001 | Syntax Error | 🌿 变异物种入侵，语法免疫系统失效 | 未知语句/表达式错误 |
| FCL-0002 | Type Mismatch | 🦴 食性冲突，捕食者拒绝进食 | 营养级差 ≠1 / 类型误用 |
...
| FCL-0010 | 超时/死循环 | ⏰ 冬眠过久（>10000 轮），强制唤醒 | HIBERNATION 不醒 |
| FCL-0011 | 表达式解析失败 | 🌿 变异物种入侵，表达式含无法解析的内容 | WITH 表达式含非法 token |
...
| ../examples/example2.fc | 20% 能量传递效率（65×0.2=13） |
...
├── fcl_error.h      # 错误码枚举 + 结构化错误
├── ecology.h/.cpp   # 生态圈：营养级、在册物种、命名校验
├── expr.h/.cpp      # 表达式求值（递归下降）
├── parser.h/.cpp    # 语句解析
